...
if options.resume:
    ...
    if options.pretrained == True:
        if not os.path.exists('model/Fine_Tuned_model'):
            os.mkdir('model/Fine_Tuned_model')
        super_folder = os.path.join('model/Fine_Tuned_model', dataset_id)
    else:
        if not os.path.exists('model/end_to_end_model'):
            os.mkdir('model/end_to_end_model')
        super_folder = os.path.join('model/end_to_end_model', dataset_id)
...
def main():
    ...
    if options.model == 1:
        model = get_model_frcnn_fpn_new_anchor(num_classes, options.pretrained, options.anchor_size)
    if options.model == 0:
    ...
    total_parameter_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
