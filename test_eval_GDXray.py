...
cnn = input("Enter 0 for mask_stAS, 1 for mask_nAS, 2 for frcnn_stAS, 3 for frcnn_nAS:")
if cnn == '0':
    cnn_1 = 'mask_r_cnn_fpn_stdAS'
if cnn == '1':
    cnn_1 = 'mask_r_cnn_fpn_newAS'
if cnn == '2':
    cnn_1 = 'faser_r_cnn_fpn_stdAS'
if cnn == '3':
    cnn_1 = 'faster_r_cnn_fpn_newAS'

score_thresh = input("Enter a score thresh (0;1):")
gen_path = '/media/pero/pero_hdd/Thesis/models/last_models/model/Fine_Tuned_model/GDXray_dataset_datasets_cropped'
model_frcnn_stdAS = 'frcnn_GDXray_dataset_datasets_cropped_AS.False_90epochs_Jul09_11-51-12/best/ckpt_epoch_39.pth'
model_mask_stdAS = 'mask_GDXray_dataset_datasets_cropped_AS.False_90epochs_Jul09_11-51-23/best/ckpt_epoch_36.pth'
model_frcnn_newAS = "frcnn_GDXray_dataset_datasets_cropped_AS.True_90epochs_Jul09_11-51-11/best/ckpt_epoch_35.pth"
model_mask_newAS = "mask_GDXray_dataset_datasets_cropped_AS.True_90epochs_Jul09_11-51-22/best/ckpt_epoch_35.pth"

dataset = '/media/pero/pero_hdd/Thesis/pytorch_frcnn_mask/Datasets/GDXray_dataset/datasets_cropped'

exp_path = 'test/GDXray'
'''
    Foldering
                '''
if not os.path.exists(os.path.join('test', 'GDXray', cnn_1)):
    os.mkdir(os.path.join('test', 'GDXray', cnn_1))
test_path = os.path.join('test', 'GDXray', cnn_1, str(score_thresh))
...
if cnn == '0':
    ...
    ckpt = torch.load(os.path.join(gen_path, model_mask_stdAS))
    model_to_test.load_state_dict(ckpt['model'])


if cnn == '1':
    model_to_test = get_model_masck_fpn_new_anchor(num_classes = 2, pretrained= True, new_AS = True)
...
result_file_path = os.path.join('test/GDXray', cnn_1, 'result_file_score{}.txt'.format(float(score_thresh)))
